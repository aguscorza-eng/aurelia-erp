...
export default function PurchasesReportTable({
  ...
  const filtered = purchases.filter((purchase:any)=>{
    const date = new Date(purchase.createdAt || Date.now());
  ...
  function concepto(purchase:any){
    const items = purchase.items || [];
    if(items.length === 0) return "Compra";
    return items.map((it:any)=>it.name || it.product?.name || "insumo").join(", ");
  }
    ...
    <div className="bg-white border rounded-3xl overflow-hidden">
      ...
      >
        <div className="text-left">
          <h2 className="text-xl font-bold">🛒 Compras del mes</h2>
        ...
        <div className="flex items-center gap-5">
          <div className="text-right">
            <p className="text-xs text-stone-500">Total comprado</p>
          ...
          <tbody>
            {filtered.map((purchase:any,index:number)=>(
              <tr key={index} className="border-t hover:bg-stone-50">
                <td className="px-6 py-4">{formatDate(purchase.createdAt)}</td>
                <td>{purchase.supplier?.name || "-"}</td>
                <td className="text-sm">{concepto(purchase)}</td>
